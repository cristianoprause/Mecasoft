...
public class StatusFuncionarioEditor extends EditorPart {
	...
	private String formatarTempo(Calendar calendarTempo){
		...
		if((calendarTempo.get(Calendar.MONTH) - 1) > 0)
			tempo = tempo.concat((calendarTempo.get(Calendar.MONTH) - 1) + " mês, ");
	...
	private Calendar calculaPeriodo(List<StatusServico> listaStatus){
		...
		// calendar para calcular o periodo entre o status de iniciado e parado começa nullo para saber no loop se ja foi encontrado algum status que
		// inicie o servico
